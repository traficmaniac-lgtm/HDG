...
## Ограничения UI-слоя
- GUI не знает детали построения цены — только отображает результат.
- GUI не меняет порядок торговых действий и не может создавать параллельные ордера.
- Все критические проверки выполняются в `TradeExecutor`.
